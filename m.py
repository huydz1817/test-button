...
# Biến toàn cục lưu process bgmi + target
bgmi_process = None
last_target = {}  # lưu target cuối cho từng user
...
def callback_handler(call):
    global bgmi_process, last_target
    ...
    if call.data == "attack":
        ...
        if user_id not in last_target:
            bot.send_message(call.message.chat.id, "⚠️ No target detected yet.")
            return
        ...
        ip = last_target[user_id]["ip"]
        port = last_target[user_id]["port"]
        time = last_target[user_id]["time"]
        ...
        if bgmi_process and bgmi_process.poll() is None:
            try:
                ...
                bot.send_message(call.message.chat.id, f"Error stopping attack: {e}")
            bgmi_process = None
        else:
            bot.send_message(call.message.chat.id, "⚠️ No running attack to stop.")
